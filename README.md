...
- 新建节点时询问节点名称和出口地址；检测到公网 IPv6 后可选择搭建 IPv4 / IPv6 双栈节点，并支持在节点管理中随时更改地址
- 支持在节点管理中设置 VPS 出站 IPv4 优先、IPv6 优先或跟随系统
...
“安装 / 初始化 sing-box”只准备基础环境。安装完成后，需要进入“代理节点管理 → 新建节点”，选择并配置至少一个协议。Shadowsocks、VLESS + Reality 和 Hysteria2 可以分别启用，并共用节点名称与出口地址。新建节点时会分别探测公网 IPv4 和 IPv6；同时检测到两种地址后，会询问是否启用双栈监听。启用后会保留 IPv4 主链接，并额外生成带 `IPv6` 标识、可直接导入的 IPv6 订阅链接。

节点管理中的“设置出站 IPv4 / IPv6 优先级”控制 VPS 访问目标域名时的地址选择，可设为 IPv4 优先、IPv6 优先或跟随系统。该设置不改变客户端连接节点所用的 IPv4/IPv6 地址；“优先”模式在首选地址族不可用时仍允许使用另一地址族。
